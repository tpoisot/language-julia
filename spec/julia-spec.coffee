describe "Julia grammar", ->
  ...
  it "tokenizes functions and types", ->
    ...
    expect(tokens[4]).toEqual value: ":", scopes: ["source.julia", "keyword.operator.ternary.julia"]
    expect(tokens[5]).toEqual value: "Int64", scopes: ["source.julia", "storage.type.julia"]
  ...
  it "tokenizes docstrings", ->
    {tokens} = grammar.tokenizeLine("@doc \"\"\" xx *x* \"\"\" ->")
